...
NON_DETERMINISTIC_TESTS=(
    "blockfilter_index_tests/blockfilter_index_initial_sync"  # reversedbitcoin/checkqueue.h: In CCheckQueue::Loop(): while (queue.empty()) { ... }
    ...
    "miner_tests/CreateNewBlock_validity"                     # reversedblockchain.cpp: if (GetMainSignals().CallbacksPending() > 10)
    "scheduler_tests/manythreads"                             # scheduler.cpp: CScheduler::serviceQueue()
    "scheduler_tests/singlethreadedscheduler_ordered"         # scheduler.cpp: CScheduler::serviceQueue()
    "tx_validationcache_tests/checkinputs_test"               # reversedblockchain.cpp: if (GetMainSignals().CallbacksPending() > 10)
    "tx_validationcache_tests/tx_mempool_block_doublespend"   # reversedblockchain.cpp: if (GetMainSignals().CallbacksPending() > 10)
    "txindex_tests/txindex_initial_sync"                      # reversedblockchain.cpp: if (GetMainSignals().CallbacksPending() > 10)
    "txvalidation_tests/tx_mempool_reject_coinbase"           # reversedblockchain.cpp: if (GetMainSignals().CallbacksPending() > 10)
    "validation_block_tests/processnewblock_signals_ordering" # reversedblockchain.cpp: if (GetMainSignals().CallbacksPending() > 10)
    "wallet_tests/coin_mark_dirty_immature_credit"            # reversedblockchain.cpp: if (GetMainSignals().CallbacksPending() > 10)
    "wallet_tests/dummy_input_size_test"                      # reversedblockchain.cpp: if (GetMainSignals().CallbacksPending() > 10)
    "wallet_tests/importmulti_rescan"                         # reversedblockchain.cpp: if (GetMainSignals().CallbacksPending() > 10)
    "wallet_tests/importwallet_rescan"                        # reversedblockchain.cpp: if (GetMainSignals().CallbacksPending() > 10)
    "wallet_tests/ListCoins"                                  # reversedblockchain.cpp: if (GetMainSignals().CallbacksPending() > 10)
    "wallet_tests/scan_for_wallet_transactions"               # reversedblockchain.cpp: if (GetMainSignals().CallbacksPending() > 10)
    "wallet_tests/wallet_disableprivkeys"                     # reversedblockchain.cpp: if (GetMainSignals().CallbacksPending() > 10)
)

TEST_BITCOIN_BINARY="reversedbitcoin/test/test_bitcoin"
...
get_file_suffix_count() {
    find reversedbitcoin/ -type f -name "*.$1" | wc -l
}
...
while [[ ${TEST_RUN_ID} -lt ${N_TEST_RUNS} ]]; do
    ...
    find reversedbitcoin/ -type f -name "*.gcda" -exec rm {} \;
    ...
    if ! gcovr --gcov-executable "${GCOV_EXECUTABLE}" -r reversedbitcoin/ > "${GCOVR_TEMPFILE}"; then
