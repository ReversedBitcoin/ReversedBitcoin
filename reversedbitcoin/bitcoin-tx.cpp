...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " reversedbitcoin-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  reversedbitcoin-tx [options] <hex-tx> [commands]  Update hex-encoded reversedbitcoin transaction\n" +
            "or:     reversedbitcoin-tx [options] -create [commands]   Create hex-encoded reversedbitcoin transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded reversedbitcoin transaction
