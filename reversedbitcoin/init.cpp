 ...
 */
static const char* BITCOIN_PID_FILENAME = "reversedbitcoind.pid";
...
{
    const std::string URL_EXPLORER= "<https://explorer.reversedbitcoin.org>";
    const std::string URL_WEBSITE = "<https://reversedbitcoin.org>";
    const std::string URL_MININGPOOL = "<https://pool.reversedbitcoin.org>";
    const std::string URL_WEBWALLET = "<https://myreversedbitcoin.org>";
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if (fs::exists(config_file_path)) {
        ...
        if (configFile != NULL) {
            std::string strHeader = "# ReversedBitcoin (RVB) config file:\n"
                                    "rpcuser=username\n"
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if reversedbitcoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if reversedbitcoin is started while in a temporary directory.\n",
