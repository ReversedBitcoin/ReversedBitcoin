...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
 ...
 //   widget->setPlaceholderText(QObject::tr("Enter a ReversedBitcoin address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no reversedbitcoin: URI
    if(!uri.isValid() || uri.scheme() != QString("reversedbitcoin"))
        return false;
...
QString formatBitcoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("reversedbitcoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
...
bool openBitcoinConf()
{
    ...
    /* Open reversedbitcoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "ReversedBitcoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "ReversedBitcoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("ReversedBitcoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for ReversedBitcoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "reversedbitcoin.desktop";
    return GetAutostartDir() / strprintf("reversedbitcoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a reversedbitcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=ReversedBitcoin\n";
        else
            optionFile << strprintf("Name=ReversedBitcoin (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(CFArrayRef listSnapshot, LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find ReversedBitcoin app
