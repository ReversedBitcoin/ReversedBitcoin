    ...
    QAbstractTableModel(parent), walletModel(parent)
{
  ...
  //  walletModel->setStyleSheet("QLabel { background-color: #040101; }");
...
QVariant RecentRequestsTableModel::data(const QModelIndex &index, int role) const
{
    if(!index.isValid() || index.row() >= list.length())
        return QVariant();
   //     QVariant()->setStyleSheet("QDisplayRole { background-color: #040101; font-weight:normal; text-align:center; font-family: Arial, sans-recif; font-size: 13px; text-transform: uppercase; color: #8C8C8C;}");
