...
const char * const BITCOIN_CONF_FILENAME = "reversedbitcoin.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "reversedbitcoind -foo=bar
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //reversedbitcoin-tx using stdin
        std::string val;
...
#else
    const char* pszModule = "reversedbitcoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ReversedBitcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ReversedBitcoin
    // Mac: ~/Library/Application Support/ReversedBitcoin
    // Unix: ~/.reversedbitcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ReversedBitcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ReversedBitcoin";
#else
    // Unix
    return pathRet / ".reversedbitcoin";
#endif
#endif
}
